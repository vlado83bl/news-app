  ...
  <dialog open>
    ...
    <section>
      <form @submit.prevent="handleSubmit">
        ...
        <button class="create__button" @click="editDocument">Kreiraj</button>
...
export default {
  emits: ['editDocument', 'closeDialog'],
  props: ['newsTitle', 'newsItem' ],
  setup(props, context) {
    const title = ref(props.newsItem.title);
    const description = ref(props.newsItem.description);
    ...
    const showEditedValue = () => {
      console.log(editedValue.title.value)
    }

    const editDocument = () => {
      context.emit('documentEdited', {
        title: editedValue.title.value,
        description: editedValue.description.value
      });
    }

    return { title, description, editDocument, editedValue, showEditedValue }
  }
};
</script>
